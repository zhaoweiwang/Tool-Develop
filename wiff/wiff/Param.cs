...
namespace wiff
{
    class Param
    {
            ...
            for (int i = 0; i < inFiles.Count(); i++){
                ...
                if (!Directory.Exists(inFiles[i]) && !File.Exists(inFiles[i])){
                    ...
                    if (fi.Extension.ToUpper().CompareTo(".WIFF") == 0){
                        inputfiles.Add(fi.ToString());      // 文件名都以 .wiff 结尾
                    }
                }else if (Directory.Exists(inFiles[i])){
                    // 是路径，检查路径中是否有 WIFF 结尾
                    bool haswiff = false;
                    ...
                    if (haswiff == false){
                        Console.WriteLine("[wiff] <Exception>: Find no WIFF files in " + inFiles[i] + "!");
                    }
                }
            }
        ...
        public void showHelpInformation(){
            ...
            Console.WriteLine("\tWiffReader.exe - extract text-plain spectra\n");
            Console.WriteLine("SYNOPSIS");
            Console.WriteLine("\tWiffReader.exe [OPTION]... [FILE]...\n");
            ...
            Console.WriteLine("\t-c");
            Console.WriteLine("\t\tMake results centroid. In default case, this switch is on.\n");
            Console.WriteLine("\t-p");
            Console.WriteLine("\t\tMake results profile. In default case, this switch is off.\n");
        }

        public void ParseParam(string[] args){          // C# 中 args[0] 不是 XXX.exe
            List<string> tmpinput = new List<string>();
            this.mgf = 0;
            
            string error = "Usage: Invoke.exe input_file_path ... [-options]\nPlease refer to the user guide for more details.\n";
            try{
                if (args.Length == 0){          // 用户双击 WiffReader.exe 时给出软件使用指导
                    showHelpInformation();
                    return;
                }
